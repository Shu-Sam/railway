...
class Railway
  ...
  def inc_speed_with_history(train_index, speed)
    train = train_by_index(train_index)
    puts 'текущая скорость поезда равна:'
    puts train.inc_speed(speed)
    puts 'история изменения скорости поезда :'
    puts train.speed_history
  end
